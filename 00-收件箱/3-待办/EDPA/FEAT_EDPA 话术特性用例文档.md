...
1. **加载机制**：兜底话术在 Agent 启动期加载，加载失败时 **fail-fast** 阻止启动，不静默运行。
2. **优先级**：兜底话术为最低优先级层，仅在任务级模板（A.2）未命中且场景级话术未覆盖时生效（见 §3 优先级模型）。
3. **键覆盖范围**：共 12 个话术配置键（tool_start/tool_end/todo_start/todo_end/todolist_start/todolist_end/interrupt_start/request_start/planning_start/task_cancelled/cancel_confirm/out_of_scope），覆盖全部事件类型。
4. **变量渲染**：`tool_start`/`tool_end` 支持 `{tool_name}` 变量；`todo_start`/`todo_end` 支持 `{title}` 变量。变量值缺失时**保留占位符原样**（如 `{tool_name}` 不替换），不替换为空字符串（见 UC-A06）。
5. **部分覆盖降级**：场景级话术仅覆盖部分键时，未覆盖的键回退到兜底话术，不因部分缺失而整体失效（见 UC-A02 备选流 A1）。
6. **键缺失容错**：兜底话术某键缺失时，记录 warn 日志，推送空字符串或跳过事件，不抛异常阻断主流程（见 UC-A01 备选流 A1）。

### A.2 工具级话术匹配模板

```yaml
query_versatile_intent_tool_text:
  "查询账户余额":
    tool_start: "正在查询账户余额..."
    tool_end: "已查询账户余额"
  "快速转账":
...
1. **匹配方式**：**精确匹配**——`tool_args["query_intent"]` 的值与 YAML 中的 key 必须完全相等才命中，大小写/空白敏感（见 UC-A03 验收标准第 5 条、澄清 Q11）。
2. **匹配 key 来源**：由 LLM 根据 SKILL.md 描述填写 `query_intent` 参数，运行时从 `tool_args["query_intent"]` 提取（见 UC-A03 前置条件第 2 条、澄清 D-09）。
3. **适用范围**：对所有业务工具生效。`call_versatile` 由 `VersatileInterruptRail` 发送 `tool_start`/`tool_end` 事件；其他工具（如 `call_mcp`）由 `ExecutionLimitRail` 发送。两个 Rail 分工不同但查同一数据源（见澄清 D-01）。
4. **降级链**：任务级精确匹配未命中 → 直接降级到 `ScriptsConfig.tool_start`/`tool_end` 兜底（两层，无场景级中间层）。降级时使用 `{tool_name}` 变量渲染（`tool_name` = `query_intent` 值）（见 UC-A03 备选流 A1、§3 优先级模型、澄清 D-05）。
5. **适用事件**：任务级模板仅影响 `tool_start`/`tool_end` 事件的内容，不影响 `todo_start`/`todo_end` 等其他事件（见澄清 E-04）。
6. **key 缺失处理**：`query_intent` 字段缺失时，跳过任务级匹配，直接降级到兜底话术（见 UC-A03 备选流 A2）。
