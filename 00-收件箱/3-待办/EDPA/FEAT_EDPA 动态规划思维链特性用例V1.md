...
>   复用说明  ：OpenJiuwen Core 的 `TodoItem` 已原生支持 `depends_on` 字段（`List<String>`）和 `status`（事件 payload 取 pending/in\_progress/completed/cancelled 四态，见 §2.3），无需扩展数据结构。`TodoTool` 已提供 `create`/`list`/`get`/`modify`（含 append/insert\_after/insert\_before）完整操作。

>   类型转换说明  ：YAML 中 `depends_on` 配置为 `int[]`（如 `[1]`），加载到 `TodoItem` 时自动转换为 `List<String>`（如 `["1"]`）。`step_id` 同理，int → String 转换在预加载阶段完成。

### 2.3 任务状态枚举

全文统一使用 OpenJiuwen Core 原生状态枚举。事件中 `status` 字段使用枚举名的小写形式（见 `EdpaEventRail.toTaskMap()`：`todo.getStatus().name().toLowerCase()`），实际会出现以下 4 个取值：

| 事件 status 值  | Java 枚举名       | 含义  | 触发时机                                              |
| ------------- | --------------- | --- | ------------------------------------------------- |
| `pending`     | `PENDING`       | 待执行 | `todo_create` 批量创建时的非首任务初始态                       |
| `in_progress` | `IN_PROGRESS`   | 执行中 | 任务开始执行（`todo_start` 发射时）                          |
| `completed`   | `COMPLETED`     | 已完成 | 任务执行完毕（`todo_end` 发射 `status=completed`）          |
| `cancelled`   | `CANCELLED`     | 已取消 | 任务被取消或依赖冲突终止（`todo_end` 发射 `status=cancelled`）   |

>   状态机  ：`planrule.yaml` 约束 LLM 仅可在以下状态间转移（严格枚举）：
...
>   枚举兼容性  ：`TodoStatus` 枚举另有 `TODO`/`DONE` 两个值，其中 `TODO` 为 `TodoItem.create()` 的默认状态（EDPAgent 主路径走 `TodoTool` 批量创建，不触发）、`DONE` 由 `TodoTool.parseStatus()` 兼容入参解析（LLM 被约束只传 `completed`，`todo_end` 亦硬编码为 `completed`）。两者在实际事件中不会出现。

>   默认状态  ：`TodoTool` 批量创建时第 1 条置为 `in_progress`，其余置为 `pending`。
...
| 7  | 逐条发送任务列表（第 1 条 status=in\_progress，其余 status=pending）                | `todolist_item`（N 次，每次 1 条任务）                                                                                             |
...
| 14 |   重新输出更新后的任务列表  （task-1 status=completed，第 2 条 status=in\_progress，其余 pending）  | `todolist_start` → `todolist_item`（N 次，每次 1 条含最新状态） → `todolist_end`                                                      |
...
| 3b | todolist 状态递进更新                 | 每轮 `todolist_item`（×N）中已完成任务 status=completed，当前执行中为 in\_progress，其余为 pending                                                                      |
...
| 4 | 任务状态流转正确    | 执行前 `status=pending`，执行后 `status=completed`        |
...
| 5  | 输出更新后的 `todolist_item`（逐条发送，已完成的标记 completed，新增的标记 pending） | `todolist_item`（N 次） |
...
| 2  | LLM 推理后决定重新规划                                            |
| 3  | EDPAgent 更新列表（step 1 标记 completed，step 2 重新标记 pending）      |
...
| 2 | 已完成任务保留 completed 状态    | 重新规划后 completed 项不被重置为 pending         |
...
| 6  | 已完成任务标记 completed，新任务标记 pending       | `todolist_item` 反映历史状态                              |
...
ReActAgent.invoke()                          ← Core: ReAct 循环启动
    ...
    │    ├─ TodoTool.create()                ← Core: 任务创建（第1个 in_progress，其余 pending）
    ...
    │         （初次任务列表创建：第 1 条 status=in_progress，其余 status=pending）
    │
    ...
    │         │        {id:"task-3", task_name:"确认交易", status:"pending"}
    │         │      ],  ← 完整数组，含最新状态，一次性发送
    │         │      timestamp
    │         │    }))
    │         └─ session.writeStream(OutputSchema("todolist_end", {timestamp}))
    │         （任务列表更新：task-1=completed，task-2=in_progress，其余 pending）
    │
...
针对 §2.3「任务状态枚举」表，依据 OpenJiuwen Core 源码与 EDPAgent 运行时实际事件发射逻辑进行核对，发现原文存在 2 处错误，已修正。

### G.2 勘误依据（代码溯源）

| # | 源码位置 | 关键证据 |
| - | --- | --- |
| 1 | `com.openjiuwen.harness.tools.TodoStatus`（Core） | 枚举共定义 6 个值：`PENDING`、`TODO`、`IN_PROGRESS`、`COMPLETED`、`DONE`、`CANCELLED` |
| 2 | `com.openjiuwen.harness.tools.TodoItem#create`（Core） | `@Builder.Default status = TODO`，为单条创建默认值 |
| 3 | `com.openjiuwen.harness.tools.TodoTool#todoItemFrom`（Core） | 批量创建：`index == 0 ? IN_PROGRESS : PENDING`，覆盖默认 `TODO` |
| 4 | `com.openjiuwen.harness.tools.TodoTool#parseStatus`（Core） | `todo_modify` 入参解析，兼容 `todo/pending/in_progress/done/completed/cancelled` 等多种写法 |
| 5 | `EdpaEventRail#toTaskMap`（EDPAgent） | `todolist_item` 事件 payload：`todo.getStatus().name().toLowerCase()`，输出**小写枚举名** |
| 6 | `EdpaEventRail#emitTodoEnds`（EDPAgent） | `todo_end` 事件 `status` 字段**硬编码**为 `"completed"`（IN_PROGRESS→COMPLETED/DONE 分支）或 `"cancelled"`（IN_PROGRESS→CANCELLED 分支），与枚举原值解耦 |
| 7 | `planrule.yaml` §状态机（EDPAgent） | 约束 LLM 仅可在 `PENDING → IN_PROGRESS → COMPLETED / CANCELLED` 之间转移（严格枚举，4 态） |

### G.3 问题清单与修复记录

| # | 问题类型 | 问题描述 | 改进建议 | 修复状态 |
| - | ---- | --- | --- | --- |
| 1 | 正确性 | §2.3 状态值列大写形式（`TODO`/`IN_PROGRESS`/`COMPLETED`/`CANCELLED`），与实际事件 payload 不符。事件 `status` 字段由 `EdpaEventRail.toTaskMap()` 经 `.name().toLowerCase()` 输出，应为小写 | 全表状态值统一改为小写（`pending`/`in_progress`/`completed`/`cancelled`） | ✅ 已修复 |
| 2 | 完整性 | §2.3 遗漏 `pending` 状态。`TodoTool.todoItemFrom` 批量创建时非首任务初始态为 `PENDING`，会经 `todolist_item` 事件发射 | 补充 `pending`（`PENDING`）行；同时新增"枚举兼容性"说明澄清 `TODO`/`DONE` 为潜在但不出现的枚举值 | ✅ 已修复 |
| 3 | 清晰性 | §2.3 缺少状态机约束与事件发射源码依据，读者无法判断"枚举共 6 个但事件只吐 4 个"的根因 | 新增"状态机"（复述 planrule.yaml 约束）、"勘误依据"代码溯源表 | ✅ 已修复 |
...
-   **事件实际吐出 4 个状态值**：`pending`、`in_progress`、`completed`、`cancelled`（均为小写）。
-   **不出现的 2 个枚举值**：`TODO`（被 `TodoTool` 批量创建覆盖）、`DONE`（LLM 被约束只传 `completed`，且 `todo_end` 硬编码 `completed`）。
-   **小写化根因**：`EdpaEventRail.toTaskMap()` 第 1431 行 `todo.getStatus().name().toLowerCase()`。
-   **硬编码特例**：`todo_end` 的 `status` 字段不经 `toLowerCase()` 转换，由 `emitTodoEnds` 直接写字符串字面量 `"completed"` / `"cancelled"`。

### G.6 第二轮全文状态值刷新记录（2026-07-21）

基于 G.3 修复决策，对正文（§1~§6）中所有事件 `status` 字段引用进行全文小写化刷新，共修订 **17 处**（不含附录 A/B 历史评审记录、§2.3 表格"Java 枚举名"列、附录 G 代码溯源中保留的 Java 枚举名）。

| # | 章节 | 行号 | 原文（节选） | 修订后（节选） |
| - | --- | --- | --- | --- |
| 1 | §2.2 复用说明 | 115 | `status`（TODO/IN\_PROGRESS/COMPLETED/CANCELLED） | `status`（事件 payload 取 pending/in\_progress/completed/cancelled 四态，见 §2.3） |
| 2 | UC-01 主流程#7 | 211 | 逐条发送任务列表（全部 status=TODO） | 逐条发送任务列表（第 1 条 status=in\_progress，其余 status=pending） |
| 3 | UC-01 主流程#12 | 216 | 第一个任务完成，标记 COMPLETED | 第一个任务完成，标记 completed |
| 4 | UC-01 主流程#14 | 218 | task-1 status=COMPLETED，其余 TODO | task-1 status=completed，第 2 条 status=in\_progress，其余 pending |
| 5 | UC-01 主流程#17 | 221 | 第二个任务完成，标记 COMPLETED | 第二个任务完成，标记 completed |
| 6 | UC-01 验收标准#3b | 273 | status=COMPLETED，当前执行中为 IN\_PROGRESS，其余为 TODO | status=completed，当前执行中为 in\_progress，其余为 pending |
| 7 | UC-02 主流程#7 | 328 | 每完成一个标记 `COMPLETED` | 每完成一个标记 `completed` |
| 8 | UC-02 验收标准#4 | 381 | 执行前 `status=TODO`，执行后 `status=COMPLETED` | 执行前 `status=pending`，执行后 `status=completed` |
| 9 | UC-03 主流程#5 | 427 | 已完成的标记 COMPLETED，新增的标记 TODO | 已完成的标记 completed，新增的标记 pending |
| 10 | UC-03 替代流程#3 | 447 | step 1 标记 COMPLETED，step 2 重新标记 TODO | step 1 标记 completed，step 2 重新标记 pending |
| 11 | UC-03 后置条件 | 456 | 已完成项保留 `COMPLETED` 状态 | 已完成项保留 `completed` 状态 |
| 12 | UC-03 验收标准#2 | 467 | 已完成任务保留 COMPLETED 状态...COMPLETED 项不被重置为 TODO | 已完成任务保留 completed 状态...completed 项不被重置为 pending |
| 13 | UC-14 主流程#6 | 1299 | 已完成任务标记 COMPLETED，新任务标记 TODO | 已完成任务标记 completed，新任务标记 pending |
| 14 | UC-14 验收标准#2 | 1310 | 已完成步骤仍为 `COMPLETED` | 已完成步骤仍为 `completed` |
| 15 | §5.1 序列模板 | 1340 | task-1=COMPLETED | task-1=completed |
| 16 | §6.4 事件发射路径 | 2362,2377,2379,2401,2421-2422,2432-2434,2439,2449 | TodoTool.create 注释、todo\_modify 入参、todolist\_item payload、状态总结注释中的大写枚举名 | 全部改为小写事件 payload 形式 |
| 17 | §6.4 事件发射路径 | 2362 | （初次任务列表创建：全部 status=TODO，第1个 IN\_PROGRESS） | （初次任务列表创建：第 1 条 status=in\_progress，其余 status=pending） |

>   保留未改的位置  ：

>   -   §2.3 表格"Java 枚举名"列（`PENDING`/`IN_PROGRESS`/`COMPLETED`/`CANCELLED`）—— 描述 Java 枚举常量，保留大写
>   -   §2.3 状态机图（`PENDING → IN_PROGRESS → COMPLETED / CANCELLED`）—— 与 `planrule.yaml` 原文一致，保留大写
>   -   附录 A/B 历史评审记录（行 1458、1482、1529、1536、1578）—— 评审快照，保留历史原貌
>   -   附录 G.2 代码溯源表、G.4 勘误前后对照表、G.5 关键结论 —— 引用 Java 枚举名或历史对照，保留大写
